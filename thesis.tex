...
\begin{proof}
 ...
 and after defining $u^\epsilon_z, w^\epsilon_z, r^\epsilon_z$ as before
 \begin{equation}
...
In the previous chapters we arrived at the a linear equation of the first kind (see \eqref{eq:lsm-approximate-lin-eq} and \eqref{eq:rg-approximate-eq}), of the form
\begin{equation}
...
we'll present a criterion such that it will be made an \emph{a posteriori} choice of $\alpha$, based on the residual $Kx^\alpha - y$, which will lead to better results.
\section{The discrepancy principle of Morozov}
Instead of solving equation $Kx=y$, the Tikhonov regularization considers the following modified equation
\begin{equation}
...
\begin{lemma}
The operator $S$ has weakly singular kernel of logarithmic type. Instead the operators $K$ and $K'$ have weakly singular kernel, which it's proved to be continuous for a boundary $\partial \Omega$ of class $C^2$, indeed for $K$ (analogously for $K'$)
\begin{equation*}
K\phi(x)\coloneqq\int_{\partial D} \phi(y) \partial_{\nu(y)} \Phi(x, y) \,dy 
=\int_{\partial D} \phi(y)\frac{1}{2\pi}\frac{1}{|x - y|}\frac{x-y}{|x-y|}\cdot\nu(y)\, dy
\end{equation*}
more explicitly in two dimension
\begin{equation}
K\phi(c(t)) =\int_0^{2\pi} \phi(c(\tau))\frac{1}{2\pi}\frac{1}{|c(t) - c(\tau)|}\frac{c(t)-c(\tau)}{|c(t) - c(\tau)|}\cdot\nu(c(\tau))|c'(\tau)|\, d\tau
\end{equation}
has diagonal term (with $\xi,\zeta \in [t,\tau]$)
\begin{align}
...
\end{lemma}
The quadrature rule substitutes 
\begin{equation}
 A\phi(c(t))=\int_0^{2\pi} K(c(t),c(\tau))\phi(c(\tau))|w'(\tau)|\,d\tau=\int_0^{2\pi}g(t,\tau)d\tau \quad t\in[0,2\pi]
\end{equation}
with its discretization, which takes in account values of density $\phi(c(t))$ in quadrature nodes $\tau_k^{(n)}$
\begin{equation}
\label{eq:quad-rule-generic}
 A_n\phi(c(t)) = \sum_{k=1}^n\alpha_k^{(n)}(t)\phi(c(\tau_k^{(n)}))
\end{equation}
For continuous kernel $K(x,y)$, it's sufficient to apply a convergent quadrature rule, like the trapezoidal rule, 
...
\begin{proposition}
 ...
 can be estimated by 
 \begin{equation}
  |E_T(g)|\leq M(\coth ns -1) \textup{ or equivalently } |E_T(g)|\leq C(g) e^{-2ns} 
 \end{equation}
...
\end{align}
with $M_2(t,\tau)$ continuous and bounded (actually both kernels $M_1, M_2$ are analytic). It's convenient to separate the two integrals in the sum
\begin{align}
...
\end{align}
We concentrate on the integral \eqref{eq:single-integral-M1}, containing the singularity, expressed as
\begin{equation}
 Qg(t)\coloneqq\frac{1}{2\pi}\int_0^{2\pi}\ln\Bigl(4\sin^2\frac{t-\tau}{2}\Bigr)g(\tau)d\tau
\end{equation}
with $g(t)$ continuous and $2\pi$-periodic. The quadrature rule $Q_ng\coloneqq Qg_n$ consists in replacing $g$ with its 
trigonometric interpolation based on equidistant quadrature nodes $\tau_j\coloneqq j\pi/n$
\begin{equation}
 Q_ng(t)\coloneqq \sum_{j=0}^{2n-1}R_j^{(n)}(t)g(\tau_j)\quad j=0,\dots, 2n-1
\end{equation}
The weights $R_j^{(n)}=QL_j$ are the integrals of the Lagrange basis's elements $L_j(\tau)$ of the interpolation previously specified.
\begin{equation}
...
Provided additional regularity of the integrand $g$, we have uniform convergence of the integrals. The following are theorems (11.6) and (11.7) in \cite{kress:book}.
\begin{proposition}
...
\end{proposition}
\begin{proposition}
 Let $g$ be an analytic $2\pi$-periodic function. Then, in the framework of assumptions \ref{assumption:holom-ext}, there exists an holomorphic extension and the estimate for the trigonometric interpolation error decrease exponentially with $n$
 \begin{equation}
...
By this way, in the case of the operators $K,K'$, the integrand function, for fixed $t$, is smooth in $(0,2\pi)$ and presents singularities at the end--points.
\begin{equation}
\label{eq:corners-double}
 K\phi(t)=\int_0^{2\pi}K(c(t),c(\tau))\phi(c(\tau))c'(\tau)\,d\tau=\int_0^{2\pi}g(t,\tau)\,d\tau
\end{equation}
The idea is to adapt the quadrature nodes such that we can neglect the end--points. We substitute the variable $\tau=w(\sigma)$ (it's convenient even to evaluate in $t=w(s)$)
\begin{equation}
\label{eq:corners-generic}
 K\phi(w(s))=\int_0^{2\pi}g(w(s),w(\sigma))w'(\sigma)\,d\sigma
\end{equation}
The function $w$ is required to be a bijection of $[0,2\pi]$ to itself, with vanishing derivatives at the end--points. In \cite{kress:bie-scattering} is cited the choice of Sag and Szekeres in \cite{Sa-Sz64}.
\begin{equation}
 w(s)=\frac{e^{-2\pi/s}}{e^{-2\pi/s} + e^{-2\pi/(2\pi - s)}}
\end{equation}
To avoid overgrading, with high convergence order effective only for large $n$, a possible substitution is suggested in \cite{kress:bie-corners}
\begin{equation}
 w(s)=2\pi\frac{[v(s)]^q}{[v(s)]^q + [v(2\pi - s)]^q}
\end{equation}
where the cubic is defined to equally distribute grid points on two sides
\begin{equation}
 v(s)=\Bigl(\frac{1}{q} - \frac{1}{2}\Bigr)\Bigl(\frac{\pi - s}{\pi}\Bigr)^3 + \frac{1}{q}\frac{s-\pi}{\pi} + \frac{1}{2}
\end{equation}
By applying trapezoidal rule on equidistant nodes, the quadrature rule for the approximate integral \eqref{eq:corners-generic} based on the graded mesh $\tau_j=w(\sigma_j)$ with equidistant $\sigma_j=j\pi/n$ is 
\begin{equation}
 K_n\phi(w(s))=\frac{\pi}{n}\sum_{j=1}^{2n-1}a_jg(w(s), w(\sigma_j))\quad a_j=w'(j\pi/n)\quad j=1,\dots,2n-1
\end{equation}
We proceed with the same way for the single layer operator $S$ defined on a boundary $\partial \Omega$ with corners. 
...
% Provided the change $\tau=w(\sigma)$, it's convenient to evaluate the single layer potential with respect the variable $t=w(s)$ (with kernel $M$ defined previously in \eqref{})
\begin{equation}
 S\phi(w(s))=\int_0^{2\pi}M(w(s),w(\sigma))\phi(c(w(\sigma)))|c'(w(\sigma))|w'(\sigma)\,d\sigma
\end{equation}
Only after variable change, we look for a convergent quadrature rule, to solve the logarithmic singularity of the kernel. Again, we isolate it
\begin{equation}
...
\begin{equation}
 \label{eq:NtoD0}
 {N_0} f (x)= S\phi_f(x)=\int_{\partial\Omega}\Phi(x,y)\phi_f(y)\,dy \quad K'\phi_f+\frac{1}{2}\phi_f= f
\end{equation}
...
\par
The \emph{Nystr\"om method} consists in substituting a convergent (according to \ref{def:convergent-quadrature-rule}) quadrature rule $A_n$ instead of $A$ in the previous \eqref{eq:nystrom-full}. 
The first discretization yields to the \emph{semi--discretized equation} finite--dimensional
\begin{equation}
\label{eq:nystrom-semi}
 \phi_n - A_n\phi_n = f
\end{equation}
\par
We will consider only the case of $\partial \Omega$ of class $C^2$, such that the integral kernel of $A$, in the next equation, is continuous (for boundaries with corners the integral kernel is weakly singular, we need to integrate separately on each smooth piece through a graded mesh).
Therefore in the sequel, quadrature operator $A_n\phi$ reduces to \eqref{eq:quad-rule-continuous} (we suppress the curve's parametrization $c$ and use quadrature nodes $x_k=c(\tau_k^{(n)})$)
\begin{equation}
...
\begin{theorem}
 Let $K(x,y)$ be a continuous integral kernel of $A$, and consider the semi--discretized equation \eqref{eq:nystrom-semi}
 \begin{equation}
  \label{eq:nystrom-semi-continuous}
  \phi_n(x) -\sum_{k=1}^n\alpha_k K(x,x_k)\phi_n(x_k)=f(x) \quad x\in \partial \Omega
 \end{equation}
 \begin{enumerate}
  \item if $\phi_n$ is a solution of \eqref{eq:nystrom-semi-continuous}, then its values $\phi_j^{(n)} = \phi_n(x_j)$ at nodes, satisfy the following linear system
  \begin{equation}
 ...
 \end{enumerate}
\end{theorem}
This fast way of solving \eqref{eq:nystrom-semi}, can be interpreted as a collocation method on the subdivision coincident with quadrature nodes.
\par
Finally the convergence result which relates the two solutions of \eqref{eq:nystrom-full} and of \eqref{eq:nystrom-semi}, guarantees us for the Nystr\"om's method.
The error analysis for the equation of the second kind is based on theorems (10.9) and (10.10) contained \cite{kress:book}, where all details can be found.
\begin{theorem}[Pointwise Convergence]
 Let $X$ be a Banach space, let $I-A:X\to X$ be injective, $A$ be linear compact, $A_n$ a sequence \emph{pointwise convergent}, of \emph{collectively compact} operators (definition is reported \ref{def:collectively-compact}), 
...
\begin{corollary}
 ...
 If $f$ is continuous, then $\phi,\phi_n$ are continuous themselves. Moreover there holds the a priori error estimate
 \begin{equation}
...
\begin{center}
\begin{figure}
\subfloat[][\emph{Convergence order of $N_0$ with $\partial \Omega$ elliptic}.]
{
\includegraphics[width=.48\textwidth]{fig/convergence_laplace_one_ellipse}
}
\subfloat[][\emph{Convergence order of $N_0$ with $\partial \Omega$ triangular}.]
{
\includegraphics[width=.48\textwidth]{fig/convergence_laplace_one_triangle}
}
\caption{Convergence order for an analytic boundary and for an Lipschitz one}
\label{fig:subfig}
\end{figure}
\end{center}
}
...
the procedure to compute the approximation $-N_{rn}f$. 
\begin{equation}
 {N_0}_nf = S_n\phi_n = SP_n\phi_n\quad and \quad{N_D}_nf = S_n{\phi_\Omega}_n + S_n{\phi_D}_n
\end{equation}
then, we will express for brevity the numerical integration as $S_n$, 
for the relative map the error estimate is
\begin{equation}
 \|{N_r}f - {N_r}_nf\|\leq\|S\|\|\phi-\phi_n\| + \|S\phi_n - S_n\phi_n\| = err_{Nystr\textit{\"{o}}m} + err_{Num\,Int}
\end{equation}
Our real intention is to construct an \emph{approximate map} $-{N_r}_n$ to generalize and speed up 
the computation to any $f$. By this way, we are forced to express $-{N_r}_n$ in terms of 
a basis $\{b_k\}$ of a $n$--dimensional subspace $X_n\subset X=L^2_0(\partial\Omega)$.
...
The original equation, which cannot always be solved, is
\begin{equation}
 N_r f = \psi_{0z}
\end{equation}
The regularized equation 
\begin{equation}
 \alpha f^\alpha + N_r^TN_r = \psi_{0z}
\end{equation}
