...
to the exact solution $N_{0n}f\to v|_{\partial \Omega}$. The integration for the ellipse 
is exact, while for a generic smooth curve we could expect a good, but not a optimal result.
...
{
\includegraphics[width=.48\textwidth]{fig/convergence_laplace_one_kite}
}
\caption{Loglog plot of the convergence order for two smooth inclusions: an ellipse, and a kite shaped domain, computed with the infinity norm.}
\label{fig:convergence-N_0-smooth}
\end{figure}
\end{center}
}
...
\subfloat[][\emph{Convergence order of $N_0$ with $\partial \Omega$ elliptic}.]
{
\includegraphics[width=.48\textwidth]{fig/convergence_laplace_one_drop}
}
\subfloat[][\emph{Convergence order of $N_0$ with $\partial \Omega$ triangular}.]
{
\includegraphics[width=.48\textwidth]{fig/convergence_laplace_one_triangle}
}
\caption{Loglog plot of the convergence order for two Lipschitz inclusions: 
a drop shaped domain with one vertex, and a triangle, computed with the infinity norm.}
...
\caption{Convergence order for an analytic boundary and for a Lipschitz one}
...
\begin{center}
\begin{figure}[]%[tb]
\subfloat[][\emph{Linear sampling method for $\alpha=1e-10$}.]
{
...
{
\includegraphics[width=.48\textwidth]{fig/discrepancy_lsm_two_ellipse0_it_alpha_0}
}
\subfloat[][\emph{Factorization method}.]
{
\includegraphics[width=.48\textwidth]{fig/discrepancy_fm_two_ellipse0}
}
\\
\subfloat[][\emph{Morozov principle applied to LSM after $15$ iterations in $\alpha$}.]
{
\includegraphics[width=.48\textwidth]{fig/discrepancy_lsm_two_ellipse0_it_alpha_14}
}
\subfloat[][\emph{Morozov principle applied to LSM after $17$ iterations in $\alpha$}.]
{
\includegraphics[width=.48\textwidth]{fig/discrepancy_lsm_two_ellipse0_it_alpha_16}
}
...
\subfloat[][\emph{Linear sampling method}.]
{
\includegraphics[width=.48\textwidth]{fig/discrepancy_lsm_three_ellipse0_it_alpha_0}
}
\subfloat[][\emph{Factorization method}.]
{
\includegraphics[width=.48\textwidth]{fig/discrepancy_fm_three_ellipse0}
}
\\
\subfloat[][\emph{Morozov principle applied to LSM after $15$ iterations in $\alpha$}.]
{
\includegraphics[width=.48\textwidth]{fig/discrepancy_lsm_three_ellipse0_it_alpha_14}
}
\subfloat[][\emph{Morozov principle applied to LSM after $17$ iterations in $\alpha$}.]
{
\includegraphics[width=.48\textwidth]{fig/discrepancy_lsm_three_ellipse0_it_alpha_16}
}
...
values of $\delta$ greater than $1e-05$, while the norm of the regularized solution computed with
the linear sampling method still approximates the unknown inclusion.

\begin{center}
\begin{figure}%[tb]
\subfloat[][\emph{Linear sampling method for $\alpha=1e-10$ and $\delta=1e-05$}.]
{
\includegraphics[width=.30\textwidth]{fig/noise_lsm_ellipse0_noiselevel1e-05}
}
\subfloat[][\emph{Linear sampling method for $\alpha=1e-10$ and $\delta=1e-02$}.]
{
\includegraphics[width=.30\textwidth]{fig/noise_lsm_ellipse0_noiselevel1e-02}
}
\subfloat[][\emph{Factorization method for $\delta=1e-02$}.]
{
