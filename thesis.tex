...
for the numerical integration of $S$. In Figure \ref{fig:contourplots_direct} we can compare the implementation of the two 
...
{
\includegraphics[width=.48\textwidth]{fig/contourplots_direct_0}
}
% \subfloat[][\emph{Computed solution for direct problem with an elliptic inclusion}.]
{
\includegraphics[width=.48\textwidth]{fig/contourplots_direct_D}
}
...
In Figure \ref{fig:convergence-N_0-Lipschitz}
with logarithmic scale for both axes, we used 
...
\par
Instead in \ref{fig:convergence-N_D-kite} we can observe the order of convergence of 
...
we have not an explicit expression. We want underline the fact that to compute a good
approximation of $N_0f$ or $N_Df$ at a certain point on the boundary $p\in\partial\Omega$, it's fundamental
to include it from the beginning, due to the singularity in the kernel of the single layer representing 
...
{
\includegraphics[width=.48\textwidth]{fig/convergence_inclusion_onekite_bemfem_bem_bemerr}
}
...
{
\includegraphics[width=.48\textwidth]{fig/convergence_inclusion_onekite_bemfem_bem}
...
% \subfloat[][\emph{Computed solution for direct Laplace problem}.]
{
\includegraphics[width=.48\textwidth]{fig/contourplots_rhs_0}
}
% \subfloat[][\emph{Computed solution for direct problem with an elliptic inclusion}.]
{
\includegraphics[width=.48\textwidth]{fig/contourplots_rhs_D}
}
